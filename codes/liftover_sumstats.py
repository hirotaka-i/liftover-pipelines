...
def sumstats_to_vcf(sumstats_file, vcf_file, chr_col, pos_col, ea_col, ref_col, 
    ...
    Args:
        ...
        source_fasta: Source reference fasta file (required)
    """
    ...
    with open(temp_vcf, 'w') as f:
        ...
        # Build INFO field with ORIG_A1 and ORIG_A2
    ...
    logging.info("Note: VCF contig warnings from bcftools can be safely ignored")
    
    # Remove uncompressed file
    os.remove(temp_vcf)
    
    # Normalize VCF against source reference
    logging.info(f"Normalizing REF alleles against source reference...")
    ...
    if result.stderr:
        logging.info("bcftools norm output:")
        logging.info(result.stderr)
    
    if result.returncode == 0:
        # Detect swaps using bcftools isec (memory efficient)
        logging.info(f"Detecting pre-liftover swaps...")
        ...
        with open(temp_with_preswap, 'w') as outf:
            ...
            cmd = ['bcftools', 'query', '-f', '%CHROM\t%POS\t%ID\t%REF\t%ALT\t%QUAL\t%FILTER\t%INFO/ORIG_A1\t%INFO/ORIG_A2\n', normalized_vcf]
        ...
        logging.info(f"Normalized VCF created with PRESWAP annotations")
    else:
        logging.warning(f"Normalization had issues, using original VCF")
        if result.stderr:
            logging.warning(f"bcftools norm stderr: {result.stderr}")
...
def update_sumstats(input_file, output_file, unmatched_file, lifted_info, rejected_ids,
    ...
    df.loc[lifted_mask, 'PRESWAP'] = df.loc[lifted_mask, '_variant_id'].map(lambda x: lifted_info[x]['preswap']).astype(bool)
...
Example:
  %(prog)s \\
    ...
    logging.info(f"Starting liftover process")
