#!/bin/bash
#
# Container wrapper for liftover_sumstats_simple.py (no pre-normalization)
...
CONTAINER_IMAGE="liftover-sumstats:latest"
...
Container wrapper for liftover_sumstats_simple.py (auto-detects Docker or Singularity)
This version does NOT normalize REF alleles before liftover - simpler tracking.
...
Note: This version assumes your input REF alleles are correct.
      For data with potentially incorrect REF alleles, use liftover_sumstats_wrapper.sh instead.
...
echo "Running liftover (simple version - no pre-normalization) using $CONTAINER_RUNTIME..." >&2
...
if [[ "$CONTAINER_RUNTIME" == "singularity" ]]; then
    ...
    singularity exec \
        ...
        --pwd "$CURRENT_DIR" \
        "$SINGULARITY_IMAGE" \
        python3 /usr/local/bin/liftover_sumstats_simple.py "${ARGS[@]}"
else
    # Docker command
    docker run --rm \
